...
const searchPokemon = debounce(async () => {
  ...
  showLoading(true);
  try {
    // Fetch a list of all Pokemon names and filter locally for partial matches
    const res = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=${maxPokemonCount}`);
    if (!res.ok) throw new Error("Failed to fetch Pokémon list");
    const data = await res.json();
    const filtered = data.results.filter(p => p.name.includes(input));
    ...
    const allPokeData = await Promise.all(
      filtered.map(p => fetch(p.url).then(res => res.json()))
    );

    allPokeData.sort((a, b) => a.id - b.id);

    for (const pokeData of allPokeData) {
      renderCard(pokeData);
    }
  } catch (error) {
    container.innerHTML = "<p style='text-align:center; font-weight:bold;'>❌ No Pokémon found.</p>";
...
sortSelect.addEventListener('change', async () => {
  ...
  if (sortValue === 'lowest' || sortValue === 'highest') {
    // If full list not loaded, load all Pokémon
    if (!fullPokemonList) {
      showLoading(true);
      try {
        const res = await fetch(`https://pokeapi.co/api/v2/pokemon?offset=0&limit=${maxPokemonCount}`);
    ...
    let sortedList = [...fullPokemonList];
    ...
    // Sort current loaded list by name
    if (!currentPokemonList || currentPokemonList.length === 0) return;
